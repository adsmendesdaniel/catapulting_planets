...
def main(): # loop principal	
	...
	while not saida: # ENQUANTO A SAIDA NAO FOR SOLICIDATADA
		...
		while not saida1: # ENQUANTO A SAIDA NAO FOR SOLICIDATADA
			...
			draw(settings)   # DESENHA VARIAVEIS
			#saida = check_exit()
			saida1 = settings['saida_q'] # VERIFICAR SAIDA, CONDICAO PARA MUSAR ELA E SAIR DO LOOP DO JOGO
...
def load(): # funcao para carregar as informacoes
	...
	angulo  = [0,0]
	p       = [620,0,600, 10] # x_barra_velocidade, x_canhao, y_canhao, velocidade da interacao
	saida_q = False
	return False, { 
...
def continuar(settings):
	...
	while not saida3 :
		...
		if  k[K_RIGHT]:	
			...
			fim = True
		if k[K_q]:
			setting['saida_q'] = True
			fim = True
	if(fim == True):
			print("entrou nos creditos ")
			import ending
	#pygame.display.flip()
...
def atualizar_posicao(todos): # todos_dados = [x, y, vx, vy, m_sol, h ]
	x, y, vx, vy, m, h, loops = todos[0], todos[1], todos[2], todos[3], todos[4], todos[5], todos[7]
	i= 0
	while(i<loops):
		x = x + h*vx
		y=  y + h*vy
	...
	#print('veio aqui')
	#print(x,y)
	return [x,y,vx,vy,todos[6],todos[7]]


def update(settings): # PARA ATUALIZAR AS VARIAVEIS
	obj = settings['obj']
	#print(obj)
	obj[0],obj[1],obj[2],obj[3],settings['saida_q'],settings['p'][3] = atualizar_posicao([obj[0],obj[1],obj[2],obj[3],obj[4],obj[5],settings['saida_q'], settings['p'][3] ])
	#print(obj)
	k = pygame.key.get_pressed()   
